...
# num_residential = 13
# store_locations = [
#     [random.randint(*x_bounds), random.randint(*y_bounds)] for _ in range(num_stores)
# ]
# residential_locations = [
#     [random.randint(*x_bounds), random.randint(*y_bounds)]
#     for _ in range(num_residential)
# ]

# Or use predefined store and residential locations used.
store_locations = [[20, 20], [20, 80], [80, 20], [80, 80]]
residential_locations = [[15, 10], [30, 90], [10, 35], [50, 40]]
...
def fitness_function(*warehouse_locations):
    ...
    for warehouse in warehouse_locations:
        store_distances.append(
            [distance(warehouse, store) for store in store_locations]
        )
        residential_distances.append(
            [distance(warehouse, res) for res in residential_locations]
        )
...
class Particle:
    ...
    Each particle has a position, velocity, and memory vector of its best position.
    """
    ...
    def update_velocity(self, global_best_position, i_weight, m_weight, s_weight):
        """Update the particle's velocity based on inertia, memory, and social influence vectors."""
        for x in range(len(self.position)):
            inertia = i_weight * self.velocity[x]
            memory = (
                m_weight * random.random() * (self.best_position[x] - self.position[x])
            )
            social = (
                s_weight
                * random.random()
                * (global_best_position[x] - self.position[x])
            )
...
warehouse_locations = [
    (swarm.global_best_position[i], swarm.global_best_position[i + 1])
    for i in range(0, len(swarm.global_best_position), 2)
]

# Plot the results
plot_results(
    x_bounds, y_bounds, store_locations, residential_locations, warehouse_locations
)
